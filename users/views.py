...
def doctor_patients(request):
    ...
    return render(
        request, "doctors/patients_list.html", {"patients": patients, "query": query}
    )
...
def add_patient(request):
    ...
    available_users = (
        CustomUser.objects.filter(is_doctor=False, is_staff=False, is_superuser=False)
        .exclude(id__in=existing_patients.values_list("id", flat=True))
    ...
    return render(
        request,
        "doctors/add_patient.html",
        {"available_users": available_users, "query": query},
    )
...
def assign_patient(request, user_id):
    ...
    if patient.is_doctor or patient.is_staff:
        messages.error(
            request, "Нельзя назначить врача или администратора как пациента"
        )
        return redirect("users:add_patient")
    ...
    if patient.attending_doctor:
        messages.warning(
            request, f"Пользователь {patient.email} уже привязан к другому врачу"
        )
    else:
