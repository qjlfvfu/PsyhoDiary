...
def diary_delete(request, pk):
    ...
    return render(request, 'psyhodiary/diary_confirm_delete.html', {'diary_entry': entry})
...
def habit_log(request, pk):
    ...
    existing_log = HabitLog.objects.filter(habit=habit, user=request.user, completed_date=today).first()

    if request.method == 'POST':
        if not existing_log:
            HabitLog.objects.create(habit=habit, user=request.user, completed_date=today)
    ...
    return render(request, 'psyhodiary/habit_log.html', {'habit': habit, 'already_logged': existing_log is not None})
...
def test_results(request):
    ...
    results_list = TestResult.objects.filter(user=request.user).order_by('-completed_at')
...
def main_page(request):
    ...
    if request.user.is_authenticated and request.user.is_doctor:
        ...
        try:
            ...
            context['unread_alerts_count'] = Alert.objects.filter(doctor=request.user, is_read=False).count()
